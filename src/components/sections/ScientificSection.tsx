      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
          ...
          >
             <div className="relative bg-white rounded-3xl shadow-2xl border border-slate-100 min-h-[600px] flex flex-col justify-center">
                ...
                <div className="flex flex-col items-center text-center">
                  ...
                  <h3 className="text-3xl font-bold text-[#05668D] mb-2">{data.awardTitle}</h3>
                  <p className="text-slate-500 mb-8">{data.awardDesc}</p>
                  <div className="w-full p-6 bg-slate-50 rounded-xl border border-slate-100 flex items-center gap-4 text-left">
                     ...
                     <div>
                       <p className="font-bold text-[#05668D]">{data.certTitle}</p>
                       <p className="text-xs text-slate-400">{data.certBody}</p>
                     </div>
                  </div>
                </div>
             </div>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
          >
            <h2 className="text-3xl md:text-4xl font-bold text-[#05668D] mb-6 whitespace-pre-line">
              {data.mainTitle}
            </h2>
            ...
            <div className="h-[400px] w-full bg-white rounded-2xl shadow-lg border border-slate-100">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                  ...
                  <Bar dataKey="before" name="Before" fill="#e2e8f0" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="after" name="After" fill="#00A896" radius={[4, 4, 0, 0]}>
